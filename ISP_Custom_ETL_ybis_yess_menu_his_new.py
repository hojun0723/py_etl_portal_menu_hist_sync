...
def PGSQLtoMSSQL():
    ...
    try:
        ...
        cnxn2 = pyodbc.connect('DRIVER={ODBC Driver 13 for SQL Server};SERVER=123.123.123.123;DATABASE=database;UID=user;PWD=password')
        ...
        */
        ...
        select t.*
        from (
        select (select max(userid) from login_log where remoteaddressip = a.remoteaddressip and left(time, 10) = left(a.time, 10) and username = a.portal_name) user_id,
	           a.mid menu_id,
	           '' menu_nm,
	           a.time his_date,
	           null type1,
        ...
        and    a."method" = 'GET'
        and    to_date(a.time, 'yyyy-mm-dd') = current_date - 1
        order  by a.time
        ) t
